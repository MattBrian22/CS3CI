...
#     map_center = [52.4862, -1.8904]  # Center of West Midlands
#     m = folium.Map(location=map_center, zoom_start=9)

#     for _, row in data.iterrows():
#         folium.Marker(
#             location=[row['Latitude'], row['Longitude']],
#             popup=f"Location: {row['Location']}<br>Current Speed: {row['Current Speed']} km/h<br>"
#                   f"Free Flow Speed: {row['Free Flow Speed']} km/h<br>Confidence: {row['Confidence'] * 100:.2f}%<br>"
#                   f"Road Closure: {'Yes' if row['Road Closure'] else 'No'}",
...
# else:
#     st.warning("No data available. Please click 'Refresh Data' to fetch traffic information.")
...
#     "Select Dashboard",
#     ["Regional dashboard", "Real time incident prediction"]
# )

# if model_selection == "Regional dashboard":
#     pass  # Existing route visualization logic
# elif model_selection == "Real time incident prediction":
#     incident()  # Assuming regional_dashboard is a valid function
